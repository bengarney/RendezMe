...
namespace Extensions
{
	...
	return coord;
		...
		Vector3d a = new Vector3d(Math.Sin(inc1*rad)*Math.Cos (Lan1*rad),Math.Sin(inc1*rad)*Math.Sin (Lan1*rad),Math.Cos(inc1*rad));
		Vector3d b = new Vector3d(Math.Sin(inc2*rad)*Math.Cos (Lan2*rad),Math.Sin(inc2*rad)*Math.Sin (Lan2*rad),Math.Cos(inc2*rad));
	...
	*/
	public static double f (this Orbit orbit, Orbit tgt_orbit,double x)
		...
		double l2= tgt_orbit.LAN;
		 ...
		 return ((a2-(e1*e1))/(e2*Math.Cos ((x+w1+l1-(w2+l2))*rad)+1)) -((a1-(e2*e2))/(e1*Math.Cos(x*rad)+1));		
